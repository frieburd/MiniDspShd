...
# Don't send volume requests to the SHD more frequently
# than this many seconds.
#
use constant SHD_VOL_REQ_MIN_PERIOD => 0.25;
...
# currVol, mute state, volIsPending, and lastSetVolTime.
my %gActiveClients;
...
sub initPlugin {
	...
	my $classSettings = Plugins::MiniDspShd::Settings->new($classPlugin);
        ...
        # Reroute all mixer volume and mute requests to us.
        $gOrigVolCmdFuncRef = Slim::Control::Request::addDispatch(['mixer', 'volume', '_newvalue'],
        							  [1, 0, 0, \&myMixerVolumeCommand]);
        $gOrigMuteCmdFuncRef = Slim::Control::Request::addDispatch(['mixer', 'muting', '_newvalue'],
        							  [1, 0, 0, \&myMixerMuteCommand]);
...
sub shutdownPlugin {
	...
	Slim::Control::Request::unsubscribe(\&newPlayerCheck);
        ...
        Slim::Control::Request::addDispatch(['mixer', 'muting', '_newvalue'], [1, 0, 0, $gOrigMuteCmdFuncRef]);
        #Slim::Control::Request::addDispatch(['getexternalvolumeinfo'], [0, 0, 0, $gOrigGetExternalVolumeInfoFuncRef]);

	# Kill timers and unsubscribe for POST events from all active client SHD's.
	foreach my $clientId (keys %gActiveClients) {
		...
		if ($gActiveClients{$client->id}{volIsPending}) {
			Slim::Utils::Timers::killTimers($client, \&sendShdPendingVol);
		}
...
sub initClient {
	...
	my $lastSetVolTime = Time::HiRes::time() - SHD_VOL_REQ_MIN_PERIOD;
	$gActiveClients{$client->id()} = { maxVol => $maxVol, shdAddr => $shdAddr, currVol => 0,
					   mute => 0, volIsPending => 0, lastSetVolTime => $lastSetVolTime };
...
# ----------------------------------------------------------------------------
sub myMixerMuteCommand {
	my $request = shift;

	_myMixerVolAndMuteCommand($request, $gOrigMuteCmdFuncRef);
}
...
sub myMixerVolumeCommand {
	...
	_myMixerVolAndMuteCommand($request, $gOrigVolCmdFuncRef);
}


# ----------------------------------------------------------------------------
# All mixer volume and mute requests are handled here.  We'll act on those we're
# enabled for and call the original (re-routed) function otherwise.
# ----------------------------------------------------------------------------
sub _myMixerVolAndMuteCommand {
	my $request = shift;
	my $origCmdFuncRef = shift;
	...
	if (!exists $gActiveClients{$client->id()}) {
		...
		eval { &{$origCmdFuncRef}($request) };
		return;
	}

	$log->debug("p0: " . $request->{'_request'}[0] . " p1: " . $request->{'_request'}[1]);

	# If this plugin made the request as a result of a POST
	# or a getState response from the SHD, now update our
	# cached values and call the original dispatch function.
	my $selfInitiated = $request->getParam('_p3');

	my $reqValue = $request->getParam('_newvalue');

	if ($selfInitiated eq SHD_PLUGIN_INITIATED) {
		$log->debug("selfInitiated trapped reqValue: " . $reqValue);

		if ($request->isCommand([['mixer'], ['volume']])) {
			# Update our cached volume.
			$log->debug("updating cached volume: " . $reqValue);
			$gActiveClients{$client->id()}{currVol} = $reqValue;
		} else {
			# Update our cached mute.
			$log->debug("updating cached mute: " . $reqValue);
			$gActiveClients{$client->id()}{mute} = $reqValue;
		}
	...
	if ($request->isCommand([['mixer'], ['volume']])) {
		...
		my $currVol = $gActiveClients{$client->id}{currVol};
		if ($shdVol != $currVol ) {
			my $pending = $gActiveClients{$client->id}{volIsPending};
			if ($pending) {
				$log->debug("vol req pending");
			       	if ($currVol != $shdVol) {
					# Replace prior pending volume value with this one.
					# We need only store the latest requested volume,
					# prior pending volume values are obsoleted by it.
					$log->debug("replacing pending vol req");
					$gActiveClients{$client->id}{currVol} = $shdVol;
				}
				eval { &{$origCmdFuncRef}($request) };
				return;
			}
			my $currTime = Time::HiRes::time();
			my $deltaTime = $currTime -  $gActiveClients{$client->id}{lastSetVolTime};
			if ($deltaTime < SHD_VOL_REQ_MIN_PERIOD) {
				# To soon after last req, make this one wait, start a timer
				# to send it later.
				$log->debug("too soon, adding pending vol req");
				$gActiveClients{$client->id}{volIsPending} = 1;
				$gActiveClients{$client->id}{currVol} = $shdVol;
				my $waitTime = SHD_VOL_REQ_MIN_PERIOD - $deltaTime;
				Slim::Utils::Timers::setTimer($client, Time::HiRes::time() + $waitTime,
							      \&sendShdPendingVol);
				eval { &{$origCmdFuncRef}($request) };
				return;
			}

			$gActiveClients{$client->id}{lastSetVolTime} = $currTime;
			$gActiveClients{$client->id}{currVol} = $shdVol;
		...
		$log->debug("muting toggle request: " . $reqValue);

		# Mute requests are expected to be infrequent, so
		# we don't bother limiting their frequency as we
		# do with volume requests.

		my $currMute = $gActiveClients{$client->id()}{mute};
		if ($currMute != $reqValue) {
			$gActiveClients{$client->id()}{mute} = $reqValue;
			sendShdMuteToggle($client, $reqValue);
		}
	}			
...
sub updateSqueezeVolAndMute {
	...
	# Update the client by simulating an LMS request. We add a param
	# so we can detect ourself in _myMixerVolumeCommand and take the
	# appropriate action.

	if ($gActiveClients{$client->id()}{currVol} != $shdVol) {
		if ($gActiveClients{$client->id()}{volIsPending}) {
			# Don't update on this event since the volume
			# will soon be modified again by the pending request.
			$log->debug("ignore volume POST, request is pending");
			return;
		}
...
# ----------------------------------------------------------------------------
# Called from a timer to send the pending volume request to the SHD.
sub sendShdPendingVol {
	my $client = shift;

	my $shdVol = $gActiveClients{$client->id}{currVol};
        $log->debug("sending pending vol req for client: " . $client->name . " vol: " . $shdVol);

	$gActiveClients{$client->id}{volIsPending} = 0;
	$gActiveClients{$client->id}{lastSetVolTime} = Time::HiRes::time();
	sendShdSetVol($client, $shdVol);
}
